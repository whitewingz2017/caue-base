...
function closePack() {
    $("body").fadeOut(250);
    $(".card").fadeOut(250)
    $.post("https://caue-tcg/close", JSON.stringify({}));
};
...
function openPack(cards) {
    ...
    $.each(cards, function (index, value) {
        setTimeout(function () {
            ...
            $(`#card${i}`).attr("class", `card ${hollow}`);
            $(`#card${i}`).css("background-image", "url(" + value.image + ")");
            $.post("https://caue-tcg/giveCard", JSON.stringify({
                card: value.card,
                hollow: value.hollow
            }));
            i++
...
function viewCard(image, hollow) {
    $("body").fadeIn(250);
    $(".card").fadeOut(0)
    $("#card3").fadeIn(250);
    let hollowClass = ""
    ...
    $("#card3").attr("class", `card ${hollowClass}`);
    $("#card3").css("background-image", "url(" + image + ")");
}