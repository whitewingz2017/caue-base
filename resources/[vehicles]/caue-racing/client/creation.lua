...
function startRaceCreation(options)
    ...
    while raceName ~= nil and options.raceName == nil and RPC.execute("caue-racing:isRaceNameTaken", raceName) == true do
...
function finishRaceCreation()
    ...
    TriggerServerEvent("caue-racing:recieveCreateData", raceName, raceType, raceThumbnail, checkpoints)
...
RegisterNetEvent("caue-racing:cmd:racecreate")
AddEventHandler("caue-racing:cmd:racecreate", function(options)
    if GetVehiclePedIsIn(PlayerPedId(), false) ~= 0 then
        startRaceCreation(options)
    end
end)

RegisterNetEvent("caue-racing:cmd:racecreatedone")
AddEventHandler("caue-racing:cmd:racecreatedone", function()
    finishRaceCreation()
end)

RegisterNetEvent("caue-racing:cmd:racecreatecancel")
AddEventHandler("caue-racing:cmd:racecreatecancel", function()
    cancelRaceCreation()
end)
