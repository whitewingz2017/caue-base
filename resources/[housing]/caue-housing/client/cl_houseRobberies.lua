...
function interactRob()
    ...
    if type(item) == "string" then
        ...
        local finished = exports["caue-taskbar"]:taskBar(timer,"Securing Item")
        ClearPedTasks(PlayerPedId())
...
RegisterNetEvent("caue-housing:alarmFail")
AddEventHandler("caue-housing:alarmFail", function()
...
RegisterNetEvent("caue-housing:alarmEnter")
AddEventHandler("caue-housing:alarmEnter", function()
    insideGame = true
...
AddEventHandler("housing:exitFrontDoor", function()
    ...
    TriggerServerEvent("caue-housing:leftHouse", Housing.currentHousingInteractions.id)
...
RegisterNetEvent("caue-housing:alarm")
AddEventHandler("caue-housing:alarm", function(pHouseId)
    if pHouseId == Housing.currentlyRobInside then
        local gameTimer = GetGameTimer() + 30000


        while true do
            local alarmOffset = exports["caue-build"]:getModule("func").currentBuildingVector()
