...
local function buyStock(stock, amount, _src)
    ...
    if not _stockData then
        TriggerClientEvent("caue-phone:notification", src, "fas fa-exclamation-circle", "Error", "Failed to retrieve stock data", 5000)
        return false
    end

    if amount > _stockData["amount"] then
        TriggerClientEvent("caue-phone:notification", src, "fas fa-exclamation-circle", "Error", "Invalid amount", 5000)
        return false
    end
    ...
    local success, message = exports["caue-financials"]:transaction(accountId, 7, value, comment, cid, 5)
    if not success then
    ...
    if not stockUpdate(stock, "amount", _stockData["amount"] - amount) then
        TriggerClientEvent("caue-phone:notification", src, "fas fa-exclamation-circle", "Error", "Stock update error", 5000)
        return false
    end
    ...
    TriggerClientEvent("caue-phone:notification", src, "fas fa-chart-bar", "Stocks", "You buyed " .. amount .. " " .. stock, 5000)
...
local function sellStock(stock, amount, _src)
    ...
    if not _stocks[stock] then
        TriggerClientEvent("caue-phone:notification", src, "fas fa-exclamation-circle", "Error", "Failed to retrieve stocks", 5000)
        return
    end
    ...
    local success, message = exports["caue-financials"]:transaction(7, accountId, value, comment, 0, 5)
    if not success then
        TriggerClientEvent("caue-phone:notification", src, "fas fa-exclamation-circle", "Error", message, 5000)
        return false
    end
    ...
    TriggerClientEvent("caue-phone:notification", src, "fas fa-chart-bar", "Stocks", "You selled " .. amount .. " " .. stock, 5000)
...
local function transferStock(stock, amount, _cid, _src)
    ...
    if cid == _cid then
        TriggerClientEvent("caue-phone:notification", src, "fas fa-exclamation-circle", "Error", "Really dude?", 5000)
        return
    end

    local exist = characterExist(_cid)
    if not exist then
        TriggerClientEvent("caue-phone:notification", src, "fas fa-exclamation-circle", "Error", "Invalid ID", 5000)
        return
    end
    ...
    TriggerClientEvent("caue-phone:notification", src, "fas fa-chart-bar", "Stocks", "You transfered " .. amount .. " " .. stock .. " to ID: " .. _cid, 5000)
...
RPC.register("caue-phone:stocksData", function(src)
    return stocksData()
end)

RPC.register("caue-phone:getStocks", function(src)
    return getStocks(src)
end)

RPC.register("caue-phone:buyStock", function(src, stock, amount)
    return buyStock(stock, amount, src)
end)

RPC.register("caue-phone:sellStock", function(src, stock, amount)
    return sellStock(stock, amount, src)
end)

RPC.register("caue-phone:transferStock", function(src, stock, amount, cid)
