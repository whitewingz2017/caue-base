...
function updateVehicleHealth()
    ...
    TriggerServerEvent("caue-vehicles:updateVehicleHealth", vid, plate, body, engine)
end
...
function getDegredation()
    ...
    local degHealth = RPC.execute("caue-vehicles:getDegradation", vid, plate)
...
AddEventHandler("caue-vehicles:randomDegredation", function(vehicle, upperLimit, spinAmount)
	...
	TriggerServerEvent("caue-vehicles:updateVehicleDegradation", vid, plate, degradations)
end)
...
AddEventHandler("caue-vehicles:examineVehicle", function(degradation)
    ...
    local mileage = RPC.execute("caue-vehicles:getMileage", vid, plate)
    local degHealth = RPC.execute("caue-vehicles:getDegradation", vid, plate)
...
AddEventHandler("caue-vehicles:repairVehicle", function(type)
    ...
    if not exports["caue-inventory"]:hasEnoughOfItem(item, 1, true) then return end
	...
	local finished = exports["caue-taskbar"]:taskBar(15000, "Repairing")
    if finished then
        if type == "body" then
        ...
        elseif type == "engine" then
            ...
            degHealth[type] = 100
            TriggerServerEvent("caue-vehicles:updateVehicleDegradation", vid, plate, degHealth)
        end
    end
        ...
        if currentVehicle ~= 0 then
            ...
            if vid or plate then
                ...
                if #(newPos - oldPos) > 10.0 then
                    ...
                    TriggerServerEvent("caue-vehicles:updateVehicleMileage", vid, plate, update)
                end
            end
        else
            Citizen.Wait(1000)
