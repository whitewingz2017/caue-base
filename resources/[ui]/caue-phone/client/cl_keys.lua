...
RegisterNUICallback('retrieveHouseKeys', function(data, cb)
    local pSharedKeys = RPC.execute("caue-phone:getHouseKeys", data.house_id)
...
RegisterNUICallback('removeHouseKey', function(data, cb)
    local removed = RPC.execute("caue-phone:removeKey", data.house_id, data.player_id)
...
RegisterNUICallback("btnMortgage", function(data, cb)
    local payed, message = RPC.execute("caue-phone:payHouse", data.house_id)
...
RegisterNUICallback("btnGiveKey", function(data, cb)
    ...
    if tped ~= nil and tped > 0 then
        local gived, message = RPC.execute("caue-phone:giveKey", data.house_id, tplyId)
        if gived then
...
RegisterNUICallback("removeSharedKey", function(data, cb)
    local removed = RPC.execute("caue-phone:removeSharedKey", data.house_id)
