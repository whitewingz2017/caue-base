        ...
        while listening do
            if IsControlJustReleased(0, 244) then
                ...
                elseif currentZone == "prison_possessions" then
                    TriggerEvent("server-inventory-open", "1", "jail-" .. exports["caue-base"]:getChar("id"))
                end
            end
            Wait(0)
        end
...
function leavePrison()
    ...
    if jailTime == 0 then
        jailed = false
        TriggerServerEvent("caue-jail:updateJailTime", -1)
...
RegisterNetEvent("caue-jail:reset")
AddEventHandler("caue-jail:reset", function(startPosition, cid, name)
    insidePrison = false
    myCell = 1
    count = 0
    jailed = false
...
AddEventHandler("caue-polyzone:exit", function(pZoneName, pZoneData)
    if pZoneName == "prison" then
        if jailed then
            ...
            insidePrison = false
            exports["caue-density"]:ChangeDensity("prison", -1)
        end
    elseif pZoneName == "prison_leave" then
...
AddEventHandler("inventory-jail", function(startPosition, cid, name)
    if insidePrison and exports["caue-inventory"]:hasEnoughOfItem("okaylockpick", 1, false) then
        TriggerServerEvent("server-inventory-open", startPosition, cid, "1", name)
    end
end)

AddEventHandler("caue-jail:sendToJail", function(pParams, pEntity, pContext)
	...
	if input["time"] then
		...
		TriggerServerEvent("caue-jail:sendToJail", GetPlayerServerId(NetworkGetPlayerIndexFromPed(pEntity)), time)
	end
end)

RegisterNetEvent("caue-jail:begInJail")
AddEventHandler("caue-jail:begInJail", function(pSkip, pTime, pName, pCid, pDate)
    myCell = math.random(#cells)

    if not pSkip then
        TriggerEvent("caue-police:uncuff")
        TriggerServerEvent("caue-jobs:changeJob", "unemployed")
        TriggerServerEvent("caue-jail:claimPossessions")
...
Citizen.CreateThread(function()
    exports["caue-density"]:RegisterDensityReason("prison", 69)

    exports["caue-polyzone"]:AddPolyZone("prison", {
    ...
    exports["caue-polyzone"]:AddBoxZone("prison_leave", vector3(1830.54, 2594.43, 46.01), 4.2, 1.4, {
        --debugPoly=true,
        heading=0,
        minZ=45.01,
        maxZ=47.61
    ...
    exports["caue-polyzone"]:AddBoxZone("prison_possessions", vector3(1840.4, 2579.31, 46.01), 0.8, 1.4, {
        --debugPoly=true,
        heading=0,
        minZ=45.01,
        maxZ=47.61
    ...
    exports["caue-polytarget"]:AddBoxZone("prison_food", vector3(1781.44, 2559.52, 45.67), 0.4, 5.0, {
        --debugPoly=true,
        heading=0,
        minZ=45.67,
        maxZ=45.87
    ...
    exports["caue-eye"]:AddPeekEntryByPolyTarget("prison_food", {{
