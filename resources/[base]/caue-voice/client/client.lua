...
AddEventHandler("mumbleConnected", function()
    ...
    TriggerEvent("caue:voice:state", true)
end)

AddEventHandler("mumbleDisconnected", function()
    print("Mumble: Disconnected")
    TriggerEvent("caue:voice:state", false)
end)

AddEventHandler("caue:voice:state", function (state)
    VoiceEnabled = state

    TriggerServerEvent("caue:voice:connection:state", state)
...
function SetVoiceProximity(proximity)
    ...
    TriggerEvent("caue:voice:proximity", proximity)
...
AddEventHandler("caue:voice:targets:player:add", AddPlayerToTargetList)

RegisterNetEvent("caue:voice:targets:player:remove")
AddEventHandler("caue:voice:targets:player:remove", RemovePlayerFromTargetList)

RegisterNetEvent("caue:voice:refresh")
AddEventHandler("caue:voice:refresh", RefreshConnection)

RegisterNetEvent("caue:voice:proximity:override")
AddEventHandler("caue:voice:proximity:override", function(pId, pMode, pRange, pPriority)
...
Citizen.CreateThread(function()
    ...
    exports["caue-keybinds"]:registerKeyMapping("", "Voice", "Reset Mumble VOIP", "+mumble", "-mumble", "")
...
RegisterNetEvent("caue-voice:setVoiceProximity")
AddEventHandler("caue-voice:setVoiceProximity", function(pProximity)
...
Citizen.CreateThread(function ()
    ...
    while true do
        ...
        if (isTalking and not isTalkingToggle) or IsTalkingOnRadio and not isTalkingOnRadio then
            ...
            TriggerEvent("caue:voice:transmissionStarted", { phone = true, radio = IsTalkingOnRadio })
        elseif (not isTalking and isTalkingToggle) or  isTalkingOnRadio and not IsTalkingOnRadio then
            ...
            TriggerEvent("caue:voice:transmissionFinished", { phone = false, radio = IsTalkingOnRadio })
        end
