...
function loadStocks()
    local stocks = RPC.execute("caue-phone:stocksData")
    local mystocks = RPC.execute("caue-phone:getStocks")
...
RegisterNUICallback("stocksTransfer", function(data, cb)
    ...
    local update = RPC.execute("caue-phone:transferStock", stock, amount, cid)
    if update then
        loadStocks()
    end
end)
...
RegisterNUICallback("stocksBuy", function(data, cb)
    ...
    local update = RPC.execute("caue-phone:buyStock", stock, amount)
    if update then
        loadStocks()
    end
end)
...
RegisterNUICallback("stocksSell", function(data, cb)
    ...
    local update = RPC.execute("caue-phone:sellStock", stock, amount)
    if update then
        loadStocks()
    end
end)