...
function SetVehicleNitroPurgeEnabled(vehicle, enabled)
    ...
    if enabled then
        ...
        for i = 0, 3 do
            local leftPurge = CreateVehiclePurgeSpray(vehicle, off.x - 0.5, off.y + 0.05, off.z + 0.03, 40.0, -20.0, 0.0, 0.5)
            local rightPurge = CreateVehiclePurgeSpray(vehicle, off.x + 0.5, off.y + 0.05, off.z + 0.03, 40.0, 20.0, 0.0, 0.5)
