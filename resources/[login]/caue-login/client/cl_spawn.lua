...
function Login.nativeStart(AlreadySpawned)
	...
	TriggerEvent("caue-hud:toggle", false)

	if not AlreadySpawned then
		tatCategory = exports["caue-clothes"]:GetTatCategs()
		tattooHashList = exports["caue-clothes"]:GetTatCategs()
		Login.custompeds = exports["caue-clothes"]:GetCustomSkins()
...
function Login.NUICallback(data)
	...
	if data.action == "currentHover" then
		...
		local coords = ScreenRelToWorld(GetCamCoord(LoginSafe.Cam), GetCamRot(LoginSafe.Cam,2),offset)
	...
	if data.action == "deleteCharacter" then
		...
		RPC.execute("caue-char:deleteCharacter", data.actionData)
	...
	if data.action == "newCharacter" then
		...
		local created = RPC.execute("caue-char:createCharacter", chardata)
		...
		if type(created) == "table" and created.err then
			TriggerEvent("DoLongHudText",created.err,2)
			Login.CreatePlayerCharacterPeds(Login.CurrentClothing,true)
			return
		end
	...
	if data.action == "favorite" then
		...
		exports["storage"]:set(fav,"cauefavorite")
	end
...
AddEventHandler("caue-base:spawnInitialized", function()
	Login.nativeStart()
end)
