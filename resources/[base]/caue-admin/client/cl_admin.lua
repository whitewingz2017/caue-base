...
function Caue.Admin.BeginAFK(self, stop)
    ...
    afk.event = AddEventHandler("caue-admin:afkStringCheck", function(text)
    ...
    Citizen.CreateThread(function()
        ...
        while true do
            ...
            if curTime - beginTime >= 300000 then TriggerServerEvent("caue-admin:Disconnect", "AFK Kick") return end
        end
    end)
end
...
RegisterNetEvent("caue-admin:afkStringCheck")

RegisterNetEvent("caue-admin:setStatus")
AddEventHandler("caue-admin:setStatus", function(src, status)
    local player = Caue._Admin.Players[src]
    if not player then return else Caue._Admin.Players[src].status = status end
end)

RegisterNetEvent("caue-admin:sendPlayerInfo")
AddEventHandler("caue-admin:sendPlayerInfo", function(data, discData)
    Caue._Admin.Players = data
    Caue._Admin.DiscPlayers = discData
end)

RegisterNetEvent("caue-admin:RemovePlayer")
AddEventHandler("caue-admin:RemovePlayer", function(src)
    local data = Caue._Admin.Players[src]
    Caue._Admin.DiscPlayers[src] = data
    Caue._Admin.Players[src] = nil
end)

RegisterNetEvent("caue-admin:AddPlayer")
AddEventHandler("caue-admin:AddPlayer", function(player)
...
function devMenu()
    ...
    if Caue.Admin:GetPlayerRank() == "dev" then
        TriggerEvent("caue-admin:openMenu")
    end
end
...
function devNoclip()
    ...
    if Caue.Admin:GetPlayerRank() == "dev" then
        ...
        TriggerEvent("caue-admin:NoclipState",bool)
...
function devCloak()
    if not devmodeToggle then return end
    if Caue.Admin:GetPlayerRank() == "dev" then
        TriggerEvent("caue-admin:CloakRemote")
    end
end
...
Citizen.CreateThread(function()
    exports["caue-keybinds"]:registerKeyMapping("", "ZZDev", "Menu", "+devMenu", "-devMenu")
    RegisterCommand('+devMenu', devMenu, false)
    RegisterCommand('-devMenu', function() end, false)

    exports["caue-keybinds"]:registerKeyMapping("", "ZZDev", "Noclip", "+devNoclip", "-devNoclip", "F2")
    RegisterCommand('+devNoclip', devNoclip, false)
    RegisterCommand('-devNoclip', function() end, false)

    exports["caue-keybinds"]:registerKeyMapping("", "ZZDev", "Cloak", "+devCloak", "-devCloak", "F3")
    RegisterCommand('+devCloak', devCloak, false)
    RegisterCommand('-devCloak', function() end, false)

    exports["caue-keybinds"]:registerKeyMapping("", "ZZDev", "Marker Teleport", "+devMarker", "-devMarker","F6")
...
function Caue.Admin.RunCommand(self, args)
    ...
    TriggerServerEvent("caue-admin:runCommand", args)
end
...
function Caue.Admin.RunNclp(self,bool)
    ...
    TriggerEvent("caue-admin:noClipToggle", isInNoclip)
end
...
RegisterNetEvent("caue-admin:RunClCommand")
AddEventHandler("caue-admin:RunClCommand", function(cmd, args)
    Caue.Admin:RunClCommand(cmd, args)
end)

RegisterNetEvent("caue-admin:updateData")
AddEventHandler("caue-admin:updateData", function(src, type, data)
...
RegisterNetEvent("caue-admin:noLongerAdmin")
AddEventHandler("caue-admin:noLongerAdmin", function()
    Caue._Admin.Players = {}
...
RegisterNetEvent("caue-admin:bringPlayer")
AddEventHandler("caue-admin:bringPlayer", function(targPos)
    local ped = PlayerPedId()
...
RegisterNetEvent("caue-admin:runSpawnCommand")
AddEventHandler("caue-admin:runSpawnCommand", function(model, livery)
    Citizen.CreateThread(function()
...
RegisterNetEvent('caue-admin:setLastVehicle')
AddEventHandler('caue-admin:setLastVehicle', function (pNetId)
...
RegisterNetEvent("caue-admin:SeatIntoLast")
AddEventHandler("caue-admin:SeatIntoLast", function()
    local rank = Caue.Admin:GetPlayerRank()
    local rankData = Caue.Admin:GetRankData(rank)
...
RegisterNetEvent("caue-admin:ReviveInDistance")
AddEventHandler("caue-admin:ReviveInDistance", function()
    local rank = Caue.Admin:GetPlayerRank()
    local rankData = Caue.Admin:GetRankData(rank)
...
RegisterNetEvent("caue-admin:cDistanceRevive")
AddEventHandler("caue-admin:cDistanceRevive", function()
