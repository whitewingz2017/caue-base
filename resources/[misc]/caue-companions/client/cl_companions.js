    ...
    createTask: (companion, interaction, params = null) => {
        ...
        const perform = () => new Promise(async (res) => {
            ...
            if (["a_c_pig", "a_c_cow"].some((model) => GetHashKey(model) === GetEntityModel(pedId))) {
                emit("caue-companions:tooBigForVehicle");
                res();
                return;
            }
            try {
...
/* harmony default export */ const companion = (async (config, interaction) => {
    ...
    const dismiss = async (lostOwnership = false) => {
        emit("caue-companions:dismissed", id, lostOwnership);
        clearInterval(aliveInterval);
        aliveInterval = null;
        if (pedId && DoesEntityExist(pedId)) {
            const networkId = NetworkGetNetworkIdFromEntity(pedId);
            emitNet("caue-companions:destroyed", networkId);
            DeleteEntity(pedId);
            pedId = null;
            return true;
        }
    ...
    emitNet("caue-companions:created", networkId);
    emit("caue-companions:summoned", id, pedId);
    return companion;
});
...
const client_interaction = interaction(() => {
    ...
    if (selectedEntity) {
        if (selectedEntityType === 1) {
            ...
            emit("caue-companions:pedSelected", selectedEntity, companion === null || companion === void 0 ? void 0 : companion.id);
        }
        else if (selectedEntityType === 2) {
            emit("caue-companions:vehicleSelected", selectedEntity);
        }
    }
    else if (selectedPosition) {
        const { x, y, z } = selectedPosition;
        emit("caue-companions:groundSelected", { x, y, z });
    }
});
...
on("gameEventTriggered", (eventName, data) => {
    ...
    if (eventName == "CEventNetworkEntityDamage" && data[0]) {
        ...
        if (companion) {
            emit("caue-companions:damageTaken", data[0], companion.id);
        }
    }
});
setTick(() => {
