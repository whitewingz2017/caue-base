...
function ConnectToATCFrequency()
    local subscribers = RPC.execute("caue-voice:atc:subscribe")
...
function DisconnectFromATCFrequency()
    ...
    RPC.execute("caue-voice:atc:unsubscribe")
...
function SetATCRadioVolume(volume, pDisableNotification)
    ...
    if not pDisableNotification then
        TriggerEvent("DoLongHudText", ("New ATC volume %s"):format(ATCRadioVolume))
    end
...
function StopATCTransmission(forced)
    ...
    ATCTimeout = TimeOut(300):next(function (continue)
        ...
        RemoveGroupFromTargetList(ATCRadio.subscribers, "atc")
...
function LoadATCModule()
    ...
    exports["caue-keybinds"]:registerKeyMapping("", "ATC", "On / Off", "+toggleATCRadioState", "-toggleATCRadioState")
    ...
    exports["caue-keybinds"]:registerKeyMapping("", "ATC", "Volume Up", "+increaseATCRadioVolume", "-increaseATCRadioVolume")
    ...
    exports["caue-keybinds"]:registerKeyMapping("", "ATC", "Volume Down", "+decreaseATCRadioVolume", "-decreaseATCRadioVolume")
    ...
    RegisterNetEvent("caue-voice:atc:connect", ConnectToATCFrequency)

    RegisterNetEvent("caue-voice:atc:disconnect", DisconnectFromATCFrequency)

    RegisterNetEvent("caue-voice:atc:setPowerState", SetATCRadioPowerState)

    RegisterNetEvent("caue-voice:atc:addSubscriber", AddATCSubscriber)

    RegisterNetEvent("caue-voice:atc:removeSubscriber", RemoveATCSubscriber)
