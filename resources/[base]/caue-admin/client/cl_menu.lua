...
function Caue.Admin.Menu.DrawJobs(self, cmd, cb)
    ...
    local jobs = exports["caue-jobs"]:getJobs()
    Citizen.CreateThread(function()
...
Citizen.CreateThread(function()
    ...
    local function DrawDev()
        ...
        if WarMenu.Button("Dev mode: " .. (Caue._Admin.Menu.DevMode and "Disable" or "Enable")) and userRank == "dev" then
            ...
            TriggerEvent("caue-admin:currentDevmode",Caue._Admin.Menu.DevMode)
        end
        if WarMenu.Button("Dev debug: " .. (Caue._Admin.Menu.DevDebug and "Disable" or "Enable")) and userRank == "dev" then
            ...
            TriggerEvent("caue-admin:currentDebug",Caue._Admin.Menu.DevDebug)
    ...
    local function DrawSearchOptions()
        ...
        local function openTextEntry(title)
            ...
            Caue.Admin.Menu:ShowTextEntry(title, "", function(result)
                ...
                TriggerServerEvent("caue-admin:searchRequest", Caue._Admin.SearchOptions.type, result)
            end)
        end
...
RegisterNetEvent("caue-admin:openMenu")
AddEventHandler("caue-admin:openMenu", function()
    WarMenu.OpenMenu("amenu")
end)

RegisterNetEvent("caue-admin:drawLastCat")
AddEventHandler("caue-admin:drawLastCat", function(sentCat)
    cat = sentCat
    WarMenu.OpenMenu("acommands")
end)

RegisterNetEvent("caue-admin:currentDebug")
AddEventHandler("caue-admin:currentDebug", function(debugToggle)
