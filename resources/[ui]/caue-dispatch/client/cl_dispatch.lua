...
AddEventHandler("caue-dispatch:manageNotifs", function(sentSetting)
...
RegisterNetEvent("caue-dispatch:911")
AddEventHandler("caue-dispatch:911", function(msg)
    local locationInfo = GetStreetAndZone()
    local gender = IsPedMale(PlayerPedId())
    ...
    if finished == 100 then
        TriggerServerEvent("dispatch:svNotify", {
            ...
            name = exports["caue-base"]:getChar("first_name") .. " " .. exports["caue-base"]:getChar("last_name"),
            number =  exports["caue-base"]:getChar("phone"),
            job = {"police"},
            information = msg
        })
    end
...
RegisterNetEvent("caue-dispatch:311")
AddEventHandler("caue-dispatch:311", function(source, args, rawCommand)
    local locationInfo = GetStreetAndZone()
    local gender = IsPedMale(PlayerPedId())
    local currentPos = GetEntityCoords(PlayerPedId())

    TriggerEvent("animation:PlayAnimation", "phone")
    local finished = exports["caue-taskbar"]:taskBar(math.random(2500, 4000), "Ligando para 311")
    TriggerEvent("animation:PlayAnimation", "cancel")
...
AddEventHandler("TriggerAIRunning",function(p)
    ...
    while dist > 3.5 and exports["caue-base"]:getVar("dead") do
        TaskGoStraightToCoord(usingped, moveto, 2.5, -1, 0.0, 0.0)
    ...
    while exports["caue-base"]:getVar("dead") do
        Citizen.Wait(1)
