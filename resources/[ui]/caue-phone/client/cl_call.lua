...
function answerPhoneCall()
    ...
    TriggerServerEvent("caue-phone:callAccept", incomingCallId)
end

function endPhoneCall()
    if isRinging then
        TriggerServerEvent("caue-phone:callDecline", incomingCallId)
        phoneNotification("fas fa-phone-volume", "Call", "Not answered", 3000)
    elseif isDialing then
        TriggerServerEvent("caue-phone:callDecline", incomingCallId)
        phoneNotification("fas fa-phone-volume", "Call", "Not answered", 3000)
    elseif activeCallId then
        TriggerServerEvent("caue-phone:callEnd", activeCallId)
...
RegisterNUICallback("callContact", function(data, cb)
    ...
    TriggerServerEvent("caue-phone:callStart", caller_number, target_number)
    closePhone()
