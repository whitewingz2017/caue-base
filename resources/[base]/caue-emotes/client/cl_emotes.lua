...
AddEventHandler("emote:setAnimsFromDB", function(expression, anim)
...
anims = {
    ...
    ["c"] = function(ped)
        if exports["caue-flags"]:HasPedFlag(PlayerPedId(), 'isSittingOnChair') then
          TriggerEvent("caue-emotes:sitOnChair")
        elseif exports["caue-flags"]:HasPedFlag(PlayerPedId(), 'isPoledancing') then
    ...
    ["umbrella"] = function(ped)
        if obj ~= 0 then
            ...
            TriggerEvent("destroyPropPerm")
            if exports["caue-inventory"]:hasEnoughOfItem("umbrella", 1) then

                TriggerEvent("actionbar:setEmptyHanded")

                local finished = exports["caue-taskbar"]:taskBar(2500,"Opening Umbrella")
    ...
    ["smoke"] = function(ped)
        if exports["caue-inventory"]:hasEnoughOfItem("ciggy", 1) then
            ClearPedTasks(PlayerPedId())
    ...
    ["cigar"] = function(ped)
        if exports["caue-inventory"]:hasEnoughOfItem("cigar", 1) then
            local animDict = "amb@world_human_smoking@male@male_a@enter"
            local animation = "enter"
            if IsPedArmed(ped, 7) then
                SetCurrentPedWeapon(ped, 0xA2719263, true)
...
AddEventHandler("animation:PlayAnimation", function(anim)
    if not exports["caue-base"]:getVar("dead") and not stuckincar then
...
function StressTest(animName)
    if (animName == "pushups" or animName == "situps" or animName == "chinups" or
        ...
        (#(GetEntityCoords(PlayerPedId()) - vector3(1744.53, 2646.94, 45.61)) <
            ...
            10.0) then
        local finished =
            exports["caue-taskbar"]:taskBar(15000, "Relieving Stress")
        if finished == 100 then
...
AddEventHandler('umbrellaLoop', function()
    ...
    while obj ~= 0 do
        ...
        if isDisabled() then TriggerEvent("animation:PlayAnimation", "umbrella") end
...
AddEventHandler("expressions", function(pArgs)
    ...
    if expressionName == "default" then
        ClearFacialIdleAnimOverride(PlayerPedId())
    else
        SetFacialIdleAnimOverride(PlayerPedId(), expressionName, 0)
    end

    TriggerServerEvent("caue-emotes:setExpData", expressionName)
