...
function ConnectToDTFrequency(pIsHeadset)
    local subscribers = RPC.execute("caue-voice:drivethru:subscribe")
...
function DisconnectFromDTFrequency()
    ...
    RPC.execute("caue-voice:drivethru:unsubscribe")
    DTIsHeadset = false
...
function SetDTRadioVolume(volume, pDisableNotification)
    ...
    if not pDisableNotification then
        TriggerEvent("DoLongHudText", ("New DriveThru volume %s"):format(DTVolume))
    end
...
function StopDTTransmission(forced)
    ...
    DTTimeout = TimeOut(300):next(function (continue)
        ...
        RemoveGroupFromTargetList(DTRadio.subscribers, "drivethru")
...
function LoadDTModule()
    ...
    exports["caue-keybinds"]:registerKeyMapping("", "DriveThru Headset", "Push-To-Talk", "+transmitToDriveThru", "-transmitToDriveThru")
    ...
    exports["caue-keybinds"]:registerKeyMapping("", "DriveThru Headset", "On / Off", "+toggleDTRadioState", "-toggleDTRadioState")
    ...
    exports["caue-keybinds"]:registerKeyMapping("", "DriveThru Headset", "Volume Up", "+increaseDTVolume", "-increaseDTVolume")
    RegisterCommand("+increaseDTVolume", IncreaseDTRadioVolume, false)
    RegisterCommand("-increaseDTVolume", function() end, false)

    exports["caue-keybinds"]:registerKeyMapping("", "DriveThru Headset", "Volume Down", "+decreaseDTRadioVolume", "-decreaseDTRadioVolume")
    ...
    RegisterNetEvent("caue-voice:drivethru:connect", ConnectToDTFrequency)

    RegisterNetEvent("caue-voice:drivethru:disconnect", DisconnectFromDTFrequency)

    RegisterNetEvent("caue-voice:drivethru:setPowerState", SetDTRadioPowerState)

    RegisterNetEvent("caue-voice:drivethru:addSubscriber", AddDTSubscriber)

    RegisterNetEvent("caue-voice:drivethru:removeSubscriber", RemoveDTSubscriber)

    -- scuffed way of overriding vanilla keybind
    AddEventHandler("caue:voice:transmissionStarted", function()
