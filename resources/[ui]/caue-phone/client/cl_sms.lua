...
function loadSMS()
    local messages = RPC.execute("caue-phone:getSMS")
    local phone = exports["caue-base"]:getChar("phone")
...
function readSMS(sender, receiver, name)
    ...
    local messages, name, phone = RPC.execute("caue-phone:readSMS", sender, receiver, name)
...
RegisterNUICallback("newMessageSubmit", function(data, cb)
    ...
    local update = RPC.execute("caue-phone:sendSMS", number, message)
    if update then
        local phone = exports["caue-base"]:getChar("phone")
