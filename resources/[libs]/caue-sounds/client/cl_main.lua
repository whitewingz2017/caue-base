...
AddEventHandler("InteractSound_CL:playAlarm", function(position, clearAlarm)
    ...
    while runAlarm do
        ...
        if time > 25000 then waiting = 1500 end
        if time > 50000 then waiting = 1000 end
        if time > 75000 then waiting = 900 end
        if time > 90000 then waiting = 500 end

        Wait(waiting)
    end

    if failedAlarm then
        TriggerEvent("caue-housing:alarmFail")
