...
function chairSit(chairPosition, cancel)
    ...
    if isSitting then
        ...
        isSitting = false
        TriggerEvent("caue-polychair:stopSitting")
    else
        ...
        TriggerEvent("caue-polychair:startSitting")
    end
    exports["caue-flags"]:SetPedFlag(ped, "isSittingOnChair", isSitting)
end
...
AddEventHandler("caue-polychair:chairSit", function(pParameters, pEntity, pContext)
    chairSit(pParameters.chairPosition, true)
end)

AddEventHandler("caue-emotes:sitOnChair", function(pArgs, pEntity, pContext)
    chairSit(nil, true)
end)
...
Citizen.CreateThread(function()
    for idx, polyTarget in ipairs(ChairZones) do
        exports["caue-polytarget"]:AddBoxZone("caue-polychair:chair_" .. tostring(idx) .. "-" .. tostring(polyTarget[4].name), polyTarget[1], polyTarget[2], polyTarget[3], polyTarget[4])
    end
end)

Citizen.CreateThread(function()
    for idx ,polyTarget in ipairs(ChairZones) do
        exports["caue-eye"]:AddPeekEntryByPolyTarget("caue-polychair:chair_" .. tostring(idx) .. "-".. tostring(polyTarget[4].name), {{
            event = "caue-polychair:chairSit",
