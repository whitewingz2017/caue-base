...
local function displayPlayerNumbers(isDisplaying)
            ...
            for id = 0, 255 do
                if NetworkIsPlayerActive(id) then
                    ...
                    if ped == playerped then
                        DrawText3DTalking(playerCoords.x, playerCoords.y, playerCoords.z + 0.5, tostring(GetPlayerServerId(id)), {152, 251, 152, 255})
                    else
                        ...
                        if distance < disPlayerNames then
                            if NetworkIsPlayerTalking(id) then
                                if cansee then
                                    DrawText3DTalking(pedCoords.x, pedCoords.y, pedCoords.z + 0.5, tostring(GetPlayerServerId(id)), {22, 55, 155, 255})
                                end
                            else
                                if cansee then
                                    DrawText3D(pedCoords.x, pedCoords.y, pedCoords.z + 0.5, tostring(GetPlayerServerId(id)), {255, 255, 255, 255})
                                end
                            end
                        end
                    end
