...
function policeEscort()
	...
	if not inmenus and exports["caue-jobs"]:getJob(job, "is_emergency") then
		local isInVeh = IsPedInAnyVehicle(PlayerPedId(), false)
		if isInVeh and exports["caue-jobs"]:getJob(job, "is_police") then
			TriggerEvent("startSpeedo")
		else
			TriggerEvent("caue-police:escort")
		end
	end
end
...
AddEventHandler("caue-police:escort", function()
	if not isDisabled() and DecorGetInt(PlayerPedId(), "escorting") == 0 and DecorGetInt(PlayerPedId(), "escorted") == 0 and DecorGetInt(PlayerPedId(), "dragging") == 0 and DecorGetInt(PlayerPedId(), "dragged") == 0 and not IsPedRagdoll(PlayerPedId()) and not IsPedInAnyVehicle(PlayerPedId(), false) and not IsPlayerFreeAiming(PlayerId()) then
		...
		if distance ~= -1 and distance < 2.0 and GetEntitySpeed(ped) < 1.0 and not IsPedInAnyVehicle(ped, false) then
			if DecorExistOn(ped, "escorted") and DecorGetInt(ped, "escorted") == 0 then
				...
				TriggerServerEvent("caue-police:escort", GetPlayerServerId(t), -1, false)
			end
		end
	else
		if DecorGetInt(PlayerPedId(), "escorting") ~= 0 then
			TriggerServerEvent("caue-police:escort", DecorGetInt(PlayerPedId(), "escorting"), 0, false)
			DecorSetInt(PlayerPedId(), "escorting", 0)
		...
		if DecorGetInt(PlayerPedId(), "dragging") ~= 0 then
			TriggerServerEvent("caue-police:escort", DecorGetInt(PlayerPedId(), "dragging"), 0, true)
...
RegisterNetEvent("caue-police:escortReceive")
AddEventHandler("caue-police:escortReceive", function(pToggle, pDrag)
	if pDrag then
...
RegisterNetEvent("caue-police:escortingReceive")
AddEventHandler("caue-police:escortingReceive", function(pToggle, pDrag)
	if pDrag then
...
RegisterNetEvent("caue-police:unescort")
AddEventHandler("caue-police:unescort", function()
	TriggerServerEvent("caue-police:escorting", DecorGetInt(PlayerPedId(), "escorted"), 0, false)
	TriggerServerEvent("caue-police:escorting", DecorGetInt(PlayerPedId(), "dragged"), 0, true)
...
RegisterNetEvent("caue-police:drag")
AddEventHandler("caue-police:drag", function()
	if not isDisabled() and DecorGetInt(PlayerPedId(), "escorting") == 0 and DecorGetInt(PlayerPedId(), "escorted") == 0 and DecorGetInt(PlayerPedId(), "dragging") == 0 and DecorGetInt(PlayerPedId(), "dragged") == 0 and not IsPedRagdoll(PlayerPedId()) and not IsPedInAnyVehicle(PlayerPedId(), false) and not IsPlayerFreeAiming(PlayerId()) then
		local t, distance, ped = GetClosestPlayer()
		...
		if distance ~= -1 and distance < 1.0 and GetEntitySpeed(ped) < 1.0 and not IsPedInAnyVehicle(ped, false) then
			if DecorExistOn(ped, "dragged") and DecorGetInt(ped, "dragged") == 0 then
				...
				TriggerServerEvent("caue-police:escort", GetPlayerServerId(t), -1, true)
			end
		end
	else
		if DecorGetInt(PlayerPedId(), "escorting") ~= 0 then
...
RegisterNetEvent("caue-police:resetEscort")
AddEventHandler("caue-police:resetEscort", function()
	if DecorGetInt(PlayerPedId(), "escorting") ~= 0 then
		TriggerServerEvent("caue-police:escort", DecorGetInt(PlayerPedId(), "escorting"), 0, false)
		DecorSetInt(PlayerPedId(), "escorting", 0)
		ClearPedTasksImmediately(PlayerPedId())
...
AddEventHandler("police:forceSeatPlayer", function(vehNet, seat)
	...
	TriggerEvent("caue-police:unescort")
	Citizen.Wait(100)
...
Citizen.CreateThread(function()
	...
	exports["caue-keybinds"]:registerKeyMapping("", "Police", "Escort / Speedo", "+policeEscort", "-policeEscort", "LEFT")

    RegisterCommand("+policeSeat", policeSeat, false)
	RegisterCommand("-policeSeat", function() end, false)
	exports["caue-keybinds"]:registerKeyMapping("", "Police", "Force into Vehicle", "+policeSeat", "-policeSeat", "RIGHT")
end)
