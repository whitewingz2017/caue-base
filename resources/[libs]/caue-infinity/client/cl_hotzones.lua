...
RegisterNetEvent("caue-infinity:hotzones:updateList")
AddEventHandler("caue-infinity:hotzones:updateList", function(zones)
...
Citizen.CreateThread(function()
    ...
    TriggerServerEvent("caue-infinity:hotzones:requestList")
    ...
    while true do
        ...
        for zoneId, zone in pairs(HotZones) do
            ...
            if isNear and not zone.inside and zone.tick >= 2 then
                zone.inside = true
                TriggerServerEvent("caue-infinity:hotzones:enteredZone", zoneId)
            ...
            elseif not isNear and zone and zone.inside and zone.tick <= 0 then
                zone.inside = false
                TriggerServerEvent("caue-infinity:hotzones:exitZone", zoneId)
