...
function readSMS(sender, receiver, name, _phone, _src)
    ...
    if _phone then
        ...
        phone = exports["caue-base"]:getChar(src, "phone")
    end
...
RPC.register("caue-phone:getSMS", function(src)
    return getSMS(src)
end)

RPC.register("caue-phone:readSMS", function(src, sender, receiver, name, phone)
    return readSMS(sender, receiver, name, phone, src)
end)

RPC.register("caue-phone:sendSMS", function(src, number, message)
