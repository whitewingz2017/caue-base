...
RegisterNetEvent("caue:peds:decor:set")
AddEventHandler("caue:peds:decor:set", function (pNetId, pType, pProperty, pValue)
    local entity = NetworkGetEntityFromNetworkId(pNetId)
...
Citizen.CreateThread(function()
    while true do
        ...
        if #toDelete > 0 then
            TriggerServerEvent("caue:peds:rogue", toDelete)
        end
