            ...
            for vehicle, enabled in pairs(Checking) do
                ...
                if inside and not Ghosted[vehicle] then
                    ...
                    if Config.antiGhostYeetVehicles then
                        TriggerEvent("caue-density:yeet", vehicle)
                    end
...
AddEventHandler("caue-density:yeet", function (vehicle)
    local vin = exports["caue-vehicles"]:GetVehicleIdentifier(vehicle)

    if vin then return end

    exports["caue-sync"]:SyncedExecution("DeleteEntity", vehicle)
end)
...
AddEventHandler("caue-config:configLoaded", function (pId)
    if (pId ~= "caue-density") then return end
