...
function InitiateMenus(isMotorcycle, vehicleHealth)
    ...
    if vehicleHealth < 1000.0 then
        ...
        TriggerServerEvent("caue-bennys:updateRepairCost", repairCost)
    ...
    if not devmode and not fullAccess then
    ...
    for k, v in ipairs(vehicleCustomisation) do
        ...
        if amountValidMods > 0 or v.id == 18 then
            if devmode or fullAccess then
                ...
                if vehclass ~= 18 then
                    -- populateMenu("mainMenu", v.id, v.category, "none")
                elseif vehclass == 18 and not v.emergencyDisabled then
                    populateMenu("mainMenu", v.id, v.category, "none")
                end
            end
        end
    ...
    if not isMotorcycle then
        if vehclass == 18 or devmode or fullAccess then
    ...
    if devmode or fullAccess or vehclass == 18 then
                ...
                if currentMenu == "repairMenu" then
                    if AttemptPurchase("repair") then
                        ...
                        if vehclass == 18 and bennysAccess == 'emergency' then
                            ...
                            local plate = exports["caue-vehicles"]:GetVehiclePlate(veh)

                            TriggerServerEvent("caue-bennys:resetDegredation", vid, plate)
