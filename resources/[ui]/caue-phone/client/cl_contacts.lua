...
AddEventHandler("SpawnEventsClient", function()
    myContacts = RPC.execute("caue-phone:getContacts")
end)
...
RegisterNUICallback("newContactSubmit", function(data, cb)
    ...
    local update = RPC.execute("caue-phone:addContact", name, number)
    if update then
        loadContacts()
    end
...
RegisterNUICallback("removeContact", function(data, cb)
    ...
    local update = RPC.execute("caue-phone:removeContact", name, number)
    if update then
        loadContacts()
    end
