...
function loadVehicle()
    local vehicles = RPC.execute("caue-phone:getVehicles")
...
RegisterNUICallback("vehtrack", function(data)
    ...
    local coords = RPC.execute("caue-phone:vehicleCoords", identifier)
...
RegisterNUICallback("vehiclePay", function(data)
    ...
    local update = RPC.execute("caue-phone:payVehicle", identifier)
    if update then
        loadVehicle()
    end
end)
...
RegisterNUICallback("carshopOutstandings", function(data)
    ...
    local logs = RPC.execute("caue-phone:carshopOutstandings", group)
    ...
    while true do
        ...
        if IsPedInAnyVehicle(PlayerPedId(), false) then
            ...
            if GetPedInVehicleSeat(vehicle, -1) == PlayerPedId() then
                local identifier = exports["caue-vehicles"]:GetVehicleIdentifier(vehicle)

                if identifier then
                    inVehicle = true
                    TriggerServerEvent("caue-phone:vehicleCoords", identifier, GetEntityCoords(vehicle))
                end
            end
        end
