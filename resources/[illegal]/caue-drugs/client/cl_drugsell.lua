...
function DrugSell(ped, oldped)
    ...
    if ped ~= oldped then
		...
		TriggerServerEvent("caue-drugs:offer", ped)
	end
end
    ...
    while true do
        ...
        if sell then
            ...
            if IsControlJustPressed(1, 306) and GetGameTimer() > SellCooldown then
				...
				TaskStandStill(ped, 100.0)
				TriggerServerEvent("caue-drugs:selling", item, price, amount, ped)
				selling = false
				sell = false
				IsSell = false
                ShowingNotification = false
	...
	while true do
        ...
        if not IsPedDeadOrDying(ped) and not IsPedInAnyVehicle(ped) then
		    ...
		    if IsPedArmed(playerPed, 7) then
		        ...
		        if not ShowingNotification then
                    ShowingNotification = true
                    exports["caue-interaction"]:showInteraction("[N] Give [G] Run away")
                end
                ...
                if IsControlJustPressed(1, 306) then
				    ...
				    DeleteObject(drug)
				    TriggerServerEvent("caue-drugs:robgive", item, amount)
				    FreezeEntityPosition(ped, false)
				    ClearPedTasks(ped)
...
RegisterNetEvent("caue-drugs:startSell")
AddEventHandler("caue-drugs:startSell", function()
...
RegisterNetEvent("caue-drugs:c_startoffers")
AddEventHandler("caue-drugs:c_startoffers", function(ped, item, price, labels, amount)
...
RegisterNetEvent("caue-drugs:canceloffers")
AddEventHandler("caue-drugs:canceloffers", function(ped)
	local playerPed = PlayerPedId()
	local vehicle = GetVehiclePedIsIn(playerPed, false)
	Citizen.Wait(3000)
	IsSell = false
...
RegisterNetEvent("caue-drugs:client:sell")
AddEventHandler("caue-drugs:client:sell", function(ped)
	IsSell = true
