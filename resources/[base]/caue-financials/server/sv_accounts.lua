...
AddEventHandler("SpawnEventsServer", function()
    ...
    local account = exports["caue-base"]:getChar(src, "bankid")
    if account == 0 then
...
RPC.register("caue-financials:getAccounts", function(src)
    if not src then return {} end

    local cid = exports["caue-base"]:getChar(src, "id")
    if not cid then return {} end

    local _groups = exports["caue-groups"]:groupsBanks(cid)
    local groups = "0"
