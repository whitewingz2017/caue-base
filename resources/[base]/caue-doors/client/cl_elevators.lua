...
function OpenElevatorMenu(pElevator, pCurrentFloor)
    ...
    exports["caue-context"]:showContext(reverse(elements))
end
...
AddEventHandler("caue-doors:elevator:prompt", function(pParameters, pEntity, pContext)
    local data = pContext.zones and pContext.zones["caue-doors:elevator"]
...
AddEventHandler("caue-doors:elevator:teleport", function (data)
    ...
    local finished = exports["caue-taskbar"]:taskBar(time, "Waiting for the Elevator", false)
...
AddEventHandler("caue-doors:elevator:access", function (data)
    ...
    TriggerServerEvent("caue-doors:change-elevator-state", elevatorId, floorId, not locked)
end)

RegisterNetEvent("caue-doors:elevators:updateState")
AddEventHandler("caue-doors:elevators:updateState", function (pElevatorId, pFloorId, pRestricted, pForceUnlock)
...
Citizen.CreateThread(function()
    ...
    Elevators = RPC.execute("caue-doors:elevators:fetch")
    ...
    for elevatorId, elevator in ipairs(Elevators) do
        ...
        for floorId, floor in ipairs(floors) do
            ...
            for zoneId, zone in ipairs(zones) do
                ...
                local data, lib = zone.options.data or {}, zone.target and "caue-polytarget" or "caue-polyzone"

                data.floorId = floorId
                data.elevatorId = elevatorId

                if zone.type == "box" then
                    exports[lib]:AddBoxZone("caue-doors:elevator", zone.center, zone.width, zone.length, zone.options)
                elseif zone.type == "circle" then
                    exports[lib]:AddCircleZone("caue-doors:elevator", zone.center, zone.radius, zone.options)
                end
            end
        end
    end

    -- PolyZoneInteraction("caue-doors:elevator:prompt", "[E] For Elevator", 38, function (data)
    --     if not data or not Elevators[data.elevatorId] then return end
    ...
    local peek = {
        group = { "caue-doors:elevator" },
        data = {
            {
                id = "elevatorPrompt",
                label = "Elevator",
                icon = "chevron-circle-up",
                event = "caue-doors:elevator:prompt",
                parameters = {},
            }
        },
        options = {
    ...
    exports["caue-eye"]:AddPeekEntryByPolyTarget(peek.group, peek.data, peek.options)
end)