...
function Apart.enterMotel(roomNumber, roomType, isSpawn)
    ...
    if not Apart.currentRoomLocks[roomType][roomNumber] and Apart.currentRoomLockDown[roomType][roomNumber] then
        if exports["caue-jobs"]:getJob(myjob, "is_police") and myjob ~= "judge" then
...
function Apart.WakeFromBed()
    local bedOffset = exports["caue-build"]:getModule("func").getCurrentBed()
...
function Apart.leaveApartment()
    ...
    if Apart.exitPoint[Apart.currentRoomType] == nil then
        exports["caue-build"]:getModule("func").exitCurrentRoom(Apart.Locations[Apart.currentRoomType][Apart.currentRoomNumber])
    else
        if type(Apart.exitPoint[Apart.currentRoomType]) == typeVector then
            exports["caue-build"]:getModule("func").exitCurrentRoom(Apart.exitPoint[Apart.currentRoomType])
        else
            local rnd = math.random(1,#Apart.exitPoint[Apart.currentRoomType])
            exports["caue-build"]:getModule("func").exitCurrentRoom(Apart.exitPoint[Apart.currentRoomType][rnd])
        end
    end
...
function Apart.openHotelStash()
    local myjob = exports["caue-base"]:getChar("job")
    if Apart.currentRoomLockDown[Apart.currentRoomType][Apart.currentRoomNumber] and (not exports["caue-jobs"]:getJob(myjob, "is_police") and myjob ~= "judge") then
    ...
    if Apart.defaultInfo[1] == Apart.currentRoomType and Apart.defaultInfo[2] == Apart.currentRoomNumber then -- owner
        ...
        if Apart.currentRoomLockDown[Apart.currentRoomType][Apart.currentRoomNumber] and (exports["caue-jobs"]:getJob(myjob, "is_police") or myjob == "judge") then
            Apart.OpenStash()
        end
    end
end
...
function garageToWorld()
    ...
    exports["caue-build"]:getModule("func").exitCurrentRoom(vector3(4.67, -724.85, 32.18))
    Apart.currentRoomNumber = Apart.defaultInfo[2]
    Apart.currentRoomType = Apart.defaultInfo[1]
    Apart.currentHotelInformation = nil
    Apart.currentGarageNumber = 0
...
AddEventHandler("apartment:removeFromBuilding", function(roomNumber,buildType)
    if Apart.currentRoomType == buildType and Apart.currentRoomNumber == roomNumber then
        if exports["caue-build"]:getModule("func").isInBuilding() then
...
AddEventHandler("apartments:menuAction", function(action)
    ...
    elseif action == "forfeit" then
        exports["caue-context"]:showContext(MenuData["apartment_check"])
    end
