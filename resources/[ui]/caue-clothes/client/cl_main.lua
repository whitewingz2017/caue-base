...
function Save(save, close, newFadeStyle)
    if save then
        ...
        TriggerEvent("caue-login:finishedClothing","Old")
        LoadPed(oldPed)
    end
...
local function listenForKeypress(zoneName, zoneData, isFree)
    ...
    Citizen.CreateThread(function()
        local tax = RPC.execute("caue-financials:priceWithTax", zoneData.basePrice, "Services")
...
RegisterNetEvent("caue-clothes:setClothes")
AddEventHandler("caue-clothes:setClothes", function(data)
    player = PlayerPedId()
...
AddEventHandler('raid_clothes:openClothing', function(pDontShowBarber, pShouldCost)
    if pShouldCost ~= nil and pShouldCost then
        local tax = RPC.execute("caue-financials:priceWithTax", MenuData.clothing_shop.basePrice, "Services")
