...
RegisterNetEvent("caue-death:dead")
AddEventHandler("caue-death:dead", function()
    lightbleed = false
    heavybleed = false
    lightestbleed = false
	lasthealth = GetEntityHealth(PlayerPedId())
...
AddEventHandler("DoDamageFunction",function(InjuryType, CurrentHitCount, boneIndex)
    ...
    if not exports["caue-base"]:getVar("dead") then
        bleeddamage = true
...
AddEventHandler("client:bed",function()
    ...
    if DoesEntityExist(objFound) then
        ...
        Citizen.CreateThread(function()
            Citizen.Wait(3000)
            exports["caue-interaction"]:showInteraction("[E] Leave Bed; [F] Dismiss")
...
AddEventHandler("Evidence:CurrentDamageListTarget",function(CurrentDamageListTarget,bt,targetid)
    ...
    if exports["caue-jobs"]:getJob(myJob, "is_police") or exports["caue-jobs"]:getJob(myJob, "is_medic") then
...
AddEventHandler("requestWounds", function(pArgs, pEntity)
    ...
    if not pEntity then
		targetPed = exports["caue-target"]:GetCurrentEntity()
	else
		targetPed = pEntity
	end
