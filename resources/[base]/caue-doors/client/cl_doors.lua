...
local function listenForKeypress()
    ...
    Citizen.CreateThread(function()
        ...
        if not hasAccess and currentDoorLockState and not isHidden then
            exports["caue-interaction"]:showInteraction("Locked", "error")
        end
        ...
        while listening do
            ...
            if currentDoorId ~= nil and hasAccess and IsControlJustReleased(0, 38) and #(GetOffsetFromEntityGivenWorldCoords(PlayerPedId(), currentDoorCoords)) <= 1.2 then
                ...
                TriggerServerEvent("caue-doors:change-lock-state", currentDoorId, not currentDoorLockState)
            end

            Wait(idle)
        end

        exports["caue-interaction"]:hideInteraction((not hasAccess or newLockState) and "error" or "success")
    end)
end
...
RegisterNetEvent("caue-doors:initial-lock-state")
AddEventHandler("caue-doors:initial-lock-state", function(pDoors)
    doors = pDoors
...
AddEventHandler("caue-doors:doorKeyFob", function()
    ...
    TriggerServerEvent("caue-doors:change-lock-state", doorId, isBollard and (not isLocked and 6 or 0) or (not isLocked))
end)

AddEventHandler("caue:target:changed", function(pEntity, pEntityType, pEntityCoords)
...
RegisterCommand("+useKeyFob", function()
    if Throttled("caue-doors:doorKeyFob", 1000) then return end
    TriggerEvent("caue-doors:doorKeyFob")
end, false)
...
Citizen.CreateThread(function()
    exports["caue-keybinds"]:registerKeyMapping("", "Vehicle", "Door Keyfob", "+useKeyFob", "-useKeyFob", "Y")

    exports["caue-polyzone"]:AddBoxZone("mrpd_bollards_01", vector3(411.66, -1027.95, 29.24), 7.8, 23.4, {
        heading=0,
        minZ=28.14,
        maxZ=32.14
    })
    exports["caue-polyzone"]:AddBoxZone("mrpd_bollards_02", vector3(411.66, -1020.09, 29.34), 7.8, 23.4, {
        heading=0,
        minZ=28.14,
        maxZ=32.14
    })
...
RegisterNetEvent("caue-doors:add")
AddEventHandler("caue-doors:add", function()
    local pEntity = exports["caue-target"]:GetEntityPlayerIsLookingAt(10.0, 2.0, 16)
    ...
    TriggerServerEvent("caue-door:add", GetEntityCoords(pEntity), GetEntityModel(pEntity))
end)
...
--     TriggerServerEvent("caue-doors:save-config", doorsCache)
-- end)